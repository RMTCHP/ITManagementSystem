(function () {
    ...
    async function openAssetBorrowingMenu(recordId) {
        const asset = state.records.find((item) => String(item.AssetID || "") === String(recordId || ""));
        if (!asset) {
            await UI.alert({ icon: "warning", title: "Asset not found", text: "Refresh the asset list and try again." });
            return;
        }
        let selectedMode = "";

        const result = await Swal.fire({
            title: "Computer Borrowing",
            html: `
                <div class="asset-borrowing-menu">
                    <div class="inventory-history__header">
                        <strong>${UI.escapeHtml(asset.AssetName || asset.AssetID)}</strong>
                        <span>${UI.escapeHtml(asset.FixedAssetNo || asset.AssetID)}</span>
                    </div>
                    <p>Select the transaction to continue.</p>
                    <div class="asset-borrowing-menu__choices">
                        <button type="button" data-borrowing-mode="borrow">
                            <i class="fa-solid fa-hand-holding-hand"></i>
                            <span><strong>Borrow Computer</strong><small>Hand over this computer to a user.</small></span>
                            <i class="fa-solid fa-arrow-right"></i>
                        </button>
                        <button type="button" data-borrowing-mode="return">
                            <i class="fa-solid fa-rotate-left"></i>
                            <span><strong>Return Computer</strong><small>Receive this computer and its accessories.</small></span>
                            <i class="fa-solid fa-arrow-right"></i>
                        </button>
                    </div>
                    <button type="button" class="ghost-btn asset-borrowing-menu__history" data-borrowing-mode="history"><i class="fa-solid fa-clock-rotate-left"></i> View borrowing history</button>
                </div>
            `,
            ...
            didOpen: () => {
                document.querySelectorAll("[data-borrowing-mode]").forEach((button) => {
                    button.addEventListener("click", () => {
                        selectedMode = button.getAttribute("data-borrowing-mode") || "";
                        Swal.clickConfirm();
                    });
                });
            },
            preConfirm: () => selectedMode || null
        });
        ...
        if (result.value === "borrow") {
            await openComputerBorrowForm(asset);
        } else if (result.value === "return") {
            await openComputerReturnForm(asset);
        } else {
            await openComputerBorrowingHistory(asset);
        }
    }

    function setupBorrowingSignature(canvas, clearButton) {
        ...
        return {
            ...
            payload: (name) => {
                ...
                return { name, type: "image/png", size: Math.ceil(dataUrl.length * 0.75), base64: dataUrl.split(",")[1] };
            }
        };
    }

    async function openComputerBorrowForm(asset) {
        let signature;
        const result = await Swal.fire({
            title: "Borrow Computer",
            html: `
                <div class="asset-borrowing-form">
                    <div class="inventory-history__header"><strong>${UI.escapeHtml(asset.AssetName || asset.AssetID)}</strong><span>${UI.escapeHtml(asset.FixedAssetNo || asset.AssetID)}</span></div>
                    <label>Borrower <input id="borrowerName" maxlength="120" required></label>
                    ...
                    <label>Accessories <input id="borrowAccessories" value="Power Adapter, Bag, Mouse" placeholder="Power Adapter, Bag, Mouse"></label>
                    <label>Software / License <textarea id="borrowSoftware" rows="3" placeholder="Windows, Office 365, or software information">Windows, Office 365, Basic software</textarea></label>
                    <div class="ticket-signature-field"><div><span>Borrower signature</span><em>Required</em><button type="button" id="clearBorrowSignature">Clear</button></div><canvas id="borrowSignatureCanvas"></canvas></div>
                </div>
            `,
            showCancelButton: true,
            showCloseButton: true,
            confirmButtonText: "Save borrowing",
            cancelButtonText: "Cancel",
            width: "min(760px, calc(100vw - 28px))",
            didOpen: () => { signature = setupBorrowingSignature(document.getElementById("borrowSignatureCanvas"), document.getElementById("clearBorrowSignature")); },
            preConfirm: () => {
        ...
        const confirmation = await UI.confirm({ title: "Confirm computer borrowing?", text: "This will assign the computer to the borrower and save the signed form.", confirmButtonText: "Confirm borrowing" });
        if (!confirmation.isConfirmed) return;
        try {
            UI.loading("Saving borrowing", "Recording handover and signature");
            await ApiClient.request("createComputerBorrowing", { token: ApiClient.getSessionToken(), ...result.value });
            Swal.close();
            await UI.alert({ icon: "success", title: "Computer borrowed", text: "The signed borrowing record has been saved." });
            await loadModuleData();
            renderTable();
        } catch (error) {
            Swal.close();
            await UI.alert({ icon: "error", title: "Unable to save borrowing", text: error.message || "Please try again." });
        }
    }
    ...
    async function openComputerReturnForm(asset) {
        ...
        try {
            ...
            Swal.close();
            await UI.alert({ icon: "error", title: "Unable to save return", text: error.message || "Please try again." });
        }
    }

    async function openComputerBorrowingHistory(asset) {
        UI.loading("Loading borrowing history", "Fetching signed borrowing records");
        let records = [];
        try {
        ...
        await Swal.fire({
            ...
            didOpen: () => {
                document.querySelectorAll("[data-borrowing-pdf]").forEach((button) => {
                    button.addEventListener("click", () => openComputerBorrowingPdf(button.getAttribute("data-borrowing-pdf")));
                });
            }
        });
    }
    ...
    async function openComputerBorrowingPdf(borrowingId) {
        ...
        printWindow.document.write("<title>Loading borrowing form...</title>");
        try {
            const result = await ApiClient.request("getComputerBorrowingPdfData", { token: ApiClient.getSessionToken(), borrowingId });
            ...
            const reportValue = (key) => key === "BorrowedAt" || key === "ReturnedAt"
                ...
                : value(key);
            const listRows = (input, rowCount, columns = 1, valueColumn = 0) => {
                const values = String(input || "").split(/\r?\n|,/).map((item) => item.trim()).filter(Boolean);
                return Array.from({ length: rowCount }, (_, index) => {
                    const cells = Array.from({ length: columns }, (__, columnIndex) => `<td>${columnIndex === valueColumn ? UI.escapeHtml(values[index] || "") : ""}</td>`).join("");
                    return `<tr>${cells}</tr>`;
                }).join("");
            };
            const softwareRows = String(record.SoftwareInfo || "").split(/\r?\n|,/).map((item) => item.trim()).filter(Boolean);
            const softwareTableRows = Array.from({ length: 7 }, (_, index) => `<tr><td class="center">${softwareRows[index] ? index + 1 : ""}</td><td>${UI.escapeHtml(softwareRows[index] || "")}</td><td></td><td></td></tr>`).join("");
            const returnItem = record.ReturnedAt ? `<tr><td>1</td><td>${value("ModelDescription")}</td><td>${value("CPU")}</td><td>${value("Storage")}</td><td>${value("RAM")}</td><td>${value("SerialNumber")}</td></tr>` : "";
            const borrowerSignature = signature(data.borrowerSignature);
            const returnSignature = signature(data.returnSignature);
            printWindow.document.open();
            if (false) {
            printWindow.document.write(`<!doctype html><html><head><title>Computer Borrowing Form ${value("BorrowingID")}</title><style>@page{size:A4 portrait;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,'Tahoma',sans-serif;color:#000;margin:0;font-size:8px;line-height:1.08}.form{width:100%}.head{height:45px;position:relative}.brand{position:absolute;top:10px;left:2px;font-size:20px;font-style:italic;font-weight:bold;letter-spacing:-1px;color:#079db4}.meta{position:absolute;right:0;top:0;text-align:right;font-size:6px;line-height:1.15}.form-title{text-align:center;font-size:10px;font-weight:bold;margin:0}.sub-title{text-align:center;font-size:8px;font-weight:bold;margin:1px 0 3px}table{width:100%;border-collapse:collapse;table-layout:fixed}th,td{border:1px solid #000;height:16px;padding:1px 3px;vertical-align:middle;overflow:hidden}th{text-align:center;font-weight:bold}.center{text-align:center}.section{font-weight:bold;font-style:italic;text-decoration:underline;border:0!important;height:16px;padding:4px 0 1px}.form-block{margin-bottom:5px}.accessory td{height:16px}.hand-over{height:84px;padding:3px!important;vertical-align:top!important;font-size:8px;line-height:2.05}.info-line{display:grid;grid-template-columns:142px 1fr 88px 1fr;align-items:center}.info-line span:nth-child(2),.info-line span:nth-child(4){border-bottom:1px dotted #000;min-height:12px;padding-left:3px}.sign-line{position:relative;display:inline-block;width:295px;min-height:15px;border-bottom:1px dotted #000;vertical-align:bottom}.sign-line img{position:absolute;left:5px;bottom:0;max-width:150px;max-height:35px}.software td,.return-table td{height:16px}.return-table{margin-top:1px}.return-info{padding:3px!important;vertical-align:top!important;line-height:1.95}.return-line{display:grid;grid-template-columns:145px 1fr 72px 1fr;align-items:center}.return-line span:nth-child(2),.return-line span:nth-child(4){border-bottom:1px dotted #000;min-height:11px}.footer-sign{border-bottom:1px dotted #000;height:13px;margin-top:2px;width:100%}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><main class="form"><div class="head"><div class="brand">RESONAC</div><div class="meta">Resonac Materials (Thailand) Co., Ltd.<br>Powdered Metal Division<br><b>NO. PM-N240....</b></div></div><div class="form-title">COMPUTER BORROWING FORM</div><div class="sub-title">ใบยืมคอมพิวเตอร์และอุปกรณ์สารสนเทศที่เกี่ยวข้อง</div><table class="form-block"><colgroup><col style="width:5%"><col style="width:21%"><col style="width:17%"><col style="width:11%"><col style="width:10%"><col style="width:8%"><col style="width:13%"></colgroup><tr><th>No</th><th>Model/Description</th><th>Fixed Asset Tag</th><th>CPU</th><th>STORAGE</th><th>RAM</th><th>S/N</th></tr><tr class="center"><td>1</td><td>${value("ModelDescription")}</td><td>${value("FixedAssetNo")}</td><td>${value("CPU")}</td><td>${value("Storage")}</td><td>${value("RAM")}</td><td>${value("SerialNumber")}</td></tr></table><table class="accessory form-block"><colgroup><col style="width:5%"><col style="width:21%"><col style="width:17%"><col style="width:11%"><col style="width:10%"><col style="width:8%"><col style="width:13%"></colgroup>${listRows(record.Accessories, 4, 7, 1)}</table><table class="form-block"><tr><td class="hand-over"><div class="info-line"><b>ชื่อผู้ส่งมอบ Handover by :</b><span>${value("HandoverBy")}</span><b>วันที่ Date :</b><span>${reportValue("BorrowedAt")}</span></div><div class="info-line"><b>ชื่อผู้รับมอบ Receive by :</b><span>${value("Borrower")}</span><b>แผนก Dept. :</b><span>${value("BorrowerDepartment")}</span></div><div class="info-line"><b>ลงชื่อ Sign</b><span class="sign-line">${borrowerSignature}</span><b>วันที่ Date :</b><span>${reportValue("BorrowedAt")}</span></div></td></tr></table><div class="section">Part 2: Information for IT</div><table class="software form-block"><colgroup><col style="width:6%"><col style="width:48%"><col style="width:25%"><col style="width:21%"></colgroup><tr><th colspan="4" style="text-align:left">Software Installation/IT maintenance</th></tr><tr><th>No</th><th>Description</th><th>Serial/Product Key</th><th>Remark</th></tr>${softwareTableRows}</table><div class="section">Part 3: Return Equipment</div><table class="return-table"><colgroup><col style="width:6%"><col style="width:28%"><col style="width:11%"><col style="width:10%"><col style="width:8%"><col style="width:15%"></colgroup><tr><th>No</th><th>Model Description</th><th>CPU</th><th>STORAGE</th><th>RAM</th><th>S/N</th></tr>${returnItem}${Array.from({length: 4}, () => '<tr><td></td><td></td><td></td><td></td><td></td><td></td></tr>').join("")}</table><table><tr><td class="return-info"><div class="return-line"><b>ผู้ส่งมอบ Return by :</b><span>${value("ReturnedBy")}</span><b>วันที่ Date :</b><span>${reportValue("ReturnedAt")}</span></div><div class="return-line"><b>ผู้รับมอบ Receive by :</b><span>${value("ReturnReceivedBy")}</span><b>วันที่ Date :</b><span>${reportValue("ReturnedAt")}</span></div><div class="return-line"><b>สภาพอุปกรณ์ :</b><span>${value("ReturnRemark")}</span><b>ลงชื่อ Sign :</b><span class="sign-line" style="width:100%">${returnSignature}</span></div><div class="footer-sign"></div></td></tr></table></main></body></html>`);
            }
            printWindow.document.close();
            const compactList = (input, emptyText) => {
            ...
            const standardSoftware = ["Windows", "Office 365", "Basic software"];
            const part2Software = String(record.SoftwareInfo || "").split(/\r?\n|,/).map((item) => item.trim()).filter(Boolean)
                .map((item) => /^office$/i.test(item) ? "Office 365" : item);
            standardSoftware.forEach((item) => {
                if (!part2Software.some((current) => current.toLowerCase() === item.toLowerCase())) {
                    part2Software.push(item);
                }
            });
            const part2AccessoryRows = Array.from({ length: Math.max(3, part2Accessories.length) }, (_, index) => `<tr><td>${part2Accessories[index] ? index + 1 : ""}</td><td>${UI.escapeHtml(part2Accessories[index] || "")}</td></tr>`).join("");
            const part2SoftwareRows = Array.from({ length: Math.max(3, part2Software.length) }, (_, index) => `<tr><td>${part2Software[index] ? index + 1 : ""}</td><td>${UI.escapeHtml(part2Software[index] || "")}</td></tr>`).join("");
            printWindow.document.open();
            printWindow.document.write(`<!doctype html><html><head><title>Computer Borrowing Form ${value("BorrowingID")}</title><style>@page{size:A4 portrait;margin:10mm}*{box-sizing:border-box}body{margin:0;color:#17324d;font:10px Arial,Tahoma,sans-serif;line-height:1.35}.header{display:flex;justify-content:space-between;border-bottom:3px solid #009db5;padding-bottom:9px}.brand{font-size:25px;font-weight:bold;font-style:italic;color:#009db5}.brand small{display:block;font-size:8px;font-style:normal;color:#607381}.meta{text-align:right;font-size:8px;color:#607381}.meta b{font-size:10px;color:#17324d}.title{display:flex;justify-content:space-between;align-items:center;margin:14px 0 11px}.title h1{margin:0;font-size:17px}.title p{margin:2px 0 0;color:#607381;font-size:9px}.status{background:${record.ReturnedAt ? "#e5f5eb;color:#15753a" : "#e1f6f9;color:#007e93"};border-radius:16px;padding:5px 11px;font-weight:bold;font-size:9px;letter-spacing:.6px}.section{border:1px solid #cbd9df;border-radius:5px;overflow:hidden;margin-top:10px;page-break-inside:avoid}.head{background:#f0f7f9;color:#00758a;padding:6px 9px;font-weight:bold;font-size:11px}.head small{float:right;color:#607381;font-weight:normal;font-size:8px}table{width:100%;border-collapse:collapse}th,td{padding:7px 9px;border-bottom:1px solid #dbe5e9;vertical-align:top;text-align:left}th{background:#fbfdfe;color:#607381;text-transform:uppercase;font-size:8px}.spec th,.spec td{text-align:center}.spec td{font-weight:bold}.grid{display:grid;grid-template-columns:1fr 1fr}.field{padding:8px 9px;border-bottom:1px solid #dbe5e9}.field:nth-child(odd){border-right:1px solid #dbe5e9}.field b{display:block;color:#607381;font-size:8px;text-transform:uppercase}.lists,.signatures{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:9px}.list,.signature{border:1px solid #dbe5e9;border-radius:4px;overflow:hidden}.list b,.signature b{display:block;padding:5px 7px;background:#fbfdfe;font-size:9px}.list td{padding:5px 7px}.list td:first-child{width:25px;text-align:center;color:#607381}.empty{text-align:center;color:#82929b;font-style:italic}.signature{min-height:70px;position:relative;padding-bottom:8px}.signature img{display:block;max-width:155px;max-height:42px;margin:4px 7px}.line{position:absolute;left:7px;right:7px;bottom:8px;border-bottom:1px solid #9babb3}.note{text-align:center;color:#71818a;font-size:8px;margin-top:10px}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><main><header class="header"><div class="brand">RESONAC<small>IT Asset Management System</small></div><div class="meta"><b>COMPUTER BORROWING RECORD</b><br>Document ID: ${value("BorrowingID")}</div></header><div class="title"><div><h1>Computer Borrowing &amp; Return</h1><p>รายละเอียดการยืม คืน และการส่งมอบอุปกรณ์คอมพิวเตอร์</p></div><span class="status">${status}</span></div><section class="section"><div class="head">Equipment details <small>ข้อมูลอุปกรณ์</small></div><table class="spec"><tr><th>Model / Description</th><th>Fixed Asset Tag</th><th>Serial Number</th></tr><tr><td>${value("ModelDescription")}</td><td>${value("FixedAssetNo")}</td><td>${value("SerialNumber")}</td></tr><tr><th>CPU</th><th>Storage</th><th>RAM</th></tr><tr><td>${value("CPU")}</td><td>${value("Storage")}</td><td>${value("RAM")}</td></tr></table></section><section class="section"><div class="head">Borrowing &amp; handover <small>ข้อมูลการส่งมอบ</small></div><div class="grid"><div class="field"><b>Handover by / ผู้ส่งมอบ</b>${value("HandoverBy")}</div><div class="field"><b>Handover date / วันที่ส่งมอบ</b>${reportValue("BorrowedAt")}</div><div class="field"><b>Borrower / ผู้ยืม</b>${value("Borrower")}</div><div class="field"><b>Department / แผนก</b>${value("BorrowerDepartment")}</div></div><div class="lists"><div class="list"><b>Accessories / อุปกรณ์ประกอบ</b><table>${compactList(record.Accessories,"No accessories recorded")}</table></div><div class="list"><b>Software / License</b><table>${compactList(record.SoftwareInfo,"No software information recorded")}</table></div></div><div class="signatures"><div class="signature"><b>Borrower signature / ลายเซ็นผู้ยืม</b>${borrowerSignature}<div class="line"></div></div><div class="signature"><b>IT handover confirmation</b><div class="line"></div></div></div></section><section class="section"><div class="head">Return details <small>ข้อมูลการคืนอุปกรณ์</small></div><div class="grid"><div class="field"><b>Returned by / ผู้คืน</b>${value("ReturnedBy")}</div><div class="field"><b>Return date / วันที่คืน</b>${reportValue("ReturnedAt")}</div><div class="field"><b>Received by / ผู้รับคืน</b>${value("ReturnReceivedBy")}</div><div class="field"><b>Condition / Remark</b>${value("ReturnRemark")}</div></div><div class="signatures"><div class="signature"><b>Returner signature / ลายเซ็นผู้คืน</b>${returnSignature}<div class="line"></div></div><div class="signature"><b>IT receiving confirmation</b><div class="line"></div></div></div></section><p class="note">This document is generated by IT Asset Management System.</p></main></body></html>`);
            printWindow.document.close();
            const part2 = printWindow.document.createElement("section");
            part2.className = "section";
            part2.innerHTML = `<div class="head">Part 2: Information for IT <small>Accessories and Software / License</small></div><div class="lists"><div class="list"><b>ACCESSORIES / อุปกรณ์ประกอบ</b><table><tr><th>No</th><th>Description</th></tr>${part2AccessoryRows}</table></div><div class="list"><b>Software / License</b><table><tr><th>No</th><th>Description</th></tr>${part2SoftwareRows}</table></div></div>`;
            ...
            printWindow.document.querySelectorAll(".signatures").forEach((signatureGroup) => {
                const itConfirmation = signatureGroup.querySelectorAll(".signature")[1];
                if (itConfirmation) itConfirmation.remove();
                signatureGroup.style.gridTemplateColumns = "1fr";
            });
            printWindow.focus();
